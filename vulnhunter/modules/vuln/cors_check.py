...
class CORSCheckScanner(BaseScanner):
    ...
    # Define a common evil origin for testing reflection
    ...
    def scan(self, forms=None, urls=None):
        ...
        # Then, check for the critical `credentials` allowance
        self._test_credentials_support(self.target_url)

        # Basic/Secondary checks
    ...
    def _get_acao_header(self, url: str, origin: str, method='GET', credentials=False) -> dict:
        ...
        try:
            ...
            if method == 'OPTIONS':
                # Preflight check requires these headers
    ...
    def _test_credentials_support(self, url):
        ...
        # Use the evil origin to trigger reflection, but specifically look for credentials flag
        result = self._get_acao_header(url, self.TEST_ORIGIN)
        acao = result.get('acao')
    ...
    def _test_wildcard(self, url):
        ...
        # Test with a non-malicious origin to see the default behavior
    ...
    def _test_options_preflight(self, url):
        ...
        if result.get('status') == 200:
            ...
            # Check for excessive methods allowed
