...
class DirbScanner(BaseScanner):
    ...
    def calibrate(self):
        ...
        for _ in range(3):
            ...
            try:
                ...
                # Store interesting properties of the wildcard response
                # We store status code and content length. 
                # Ideally we might want fuzzy hashing but length + code is a good start.
        ...
        if non_404s:
            ...
            for r in non_404s:
                self.logger.raw(f"    - Status: {r['status']}, Length: {r['length']}")
    ...
    def is_wildcard(self, status_code, content_length):
        """
        ...
        for w in self.wildcard_responses:
            ...
            # (unless the server returns 404 for valid files too, which is rare/weird).
            if w['status'] == 404:
                continue
            
            # Strict status match
            if status_code == w['status']:
                # Loose length match (allow small variance, e.g. dynamic timestamps)
                # For now, let's say within 5% or 5 bytes difference (whichever is larger, but be careful with small files)
                diff = abs(content_length - w['length'])
                
                # If the file is very small (< 100 bytes), be strict (max 5 bytes diff)
                if w['length'] < 100:
                    if diff <= 5:
                        return True
                # For larger files, allow 5% variance
    ...
    def scan_base(self, base_url, words, depth, max_depth):
        ...
        def check_word(word):
            ...
            try:
                ...
                local_downloaded += 1
                if resp.status_code not in [404, 403, 301, 307, 308, 429] and not self.is_wildcard(resp.status_code, len(resp.content)):
            ...
            # Try as directory (skip if word has file extension or file was reported)
            if '.' not in word and not file_reported:
